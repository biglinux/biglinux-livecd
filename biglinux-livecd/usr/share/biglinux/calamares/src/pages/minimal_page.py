...
class MinimalPage(Gtk.Box):
    ...
    def create_content(self):
        ...
        status_label = Gtk.Label(
            label=_("Loading packages..."), halign=Gtk.Align.CENTER
        )
    ...
    def on_packages_load_error(self, error_message):
        ...
        self.loading_status_label.set_text(error_text)
        self.loading_status_label.add_css_class("error")
    ...
    def set_all_selected(self, selected: bool):
        ...
        for name, row in self.package_rows.items():
            ...
            if isinstance(switch, Gtk.Switch) and switch.get_active() != selected:
    ...
    def do_continue_action(self, button):
        ...
        if not packages_to_remove:
            self.logger.info(
                "No programs selected for removal. Proceeding with standard installation."
            )
        ...
        try:
            success = self.install_service.start_installation(
                filesystem_type="btrfs", packages_to_remove=packages_to_remove
            )
            if success:
            ...
            self.logger.error(
                f"Failed to configure minimal installation: {e}", exc_info=True
            )
            reset_button_state()
